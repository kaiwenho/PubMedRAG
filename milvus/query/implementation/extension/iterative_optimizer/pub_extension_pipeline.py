...
class PipelineConfig:
    ...
    def __init__(self, predicate: str, results_path: str = 'result/iterative_batches/evaluation_results.json'):
...
class PubExtensionPipeline:
    ...
    def _calculate_pipeline_statistics(
        ...
        FIXED VERSION: Handles empty DataFrames and missing columns properly.

        Args:
            edge_contexts: Dict mapping edge_id -> list of search results
            supporting_pmids: Dict mapping edge_id -> list of PMIDs passing classification
            supporting_results: DataFrame with validation results (yes/maybe)
            non_supporting_results: DataFrame with validation results (no)

        Returns:
            Dictionary with per_run statistics and per_edge details
        """
        import pandas as pd
        ...
        # Check if we have validation results with proper structure
        has_validation_results = (
            has_required_columns(supporting_results, ['edge_index', 'pmid', 'abstract_support?']) or
            has_required_columns(non_supporting_results, ['edge_index', 'pmid', 'abstract_support?'])
        )
        ...
        for edge_id in edge_contexts.keys():
            # Extract PMIDs from semantic search results
            ...
            # FIX: Check if validation was run AND has proper structure
            if has_validation_results:
                try:
                    # Safely filter supporting_results for this edge
                    if has_required_columns(supporting_results, ['edge_index', 'pmid', 'abstract_support?']):
                        ...
                        # No supporting results with proper structure
                        edge_validation = pd.DataFrame()
                        edge_yes = pd.DataFrame()

                    # Get PMIDs from validation results
                    if not edge_validation.empty:
                        ...
                        # No validation results for this edge
                        edge_stat.update({
                            'validation_pmids': 0,
                            'validation_yes_pmids': 0,
                            'ac_to_llm_rate': 0.0
                        })

                except Exception as e:
                    # Handle any unexpected errors gracefully
                    print(f"Warning: Error processing validation for edge {edge_id}: {e}")
                    edge_stat.update({
                        'validation_pmids': 0,
                        'validation_yes_pmids': 0,
                        'ac_to_llm_rate': 0.0
        ...
        # Check if validation was run by checking if we have results with proper structure
        if has_validation_results:
            try:
                # Count validation results
                total_validation_pmids = 0
                total_yes_pmids = 0

                if has_required_columns(supporting_results, ['edge_index', 'pmid', 'abstract_support?']):
                    total_validation_pmids += len(supporting_results)
                    total_yes_pmids += len(supporting_results[supporting_results['abstract_support?'] == 'yes'])

                if has_required_columns(non_supporting_results, ['edge_index', 'pmid', 'abstract_support?']):
                    total_validation_pmids += len(non_supporting_results)
                    # non_supporting_results should not have 'yes', but count them if they do
                    if 'abstract_support?' in non_supporting_results.columns:
                        total_yes_pmids += len(non_supporting_results[non_supporting_results['abstract_support?'] == 'yes'])
                ...
                print(f"Warning: Error calculating per-run validation statistics: {e}")
                statistics['per_run']['validation_total_pmids'] = 0
                statistics['per_run']['validation_yes_pmids'] = 0
                statistics['per_run']['ac_to_llm_success_rate'] = 0.0
